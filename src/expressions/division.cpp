...
std::unique_ptr<VarType> ASTExpressionDivision::ReturnType(ASTFunction* func)
{
    ...
    {
        if (!ASTExpression::CoerceMathTypes(func, d1, d2, returnType)) // This will force our arguments to be the same type and outputs which one it is.
...
llvm::Value* ASTExpressionDivision::Compile(llvm::IRBuilder<>& builder, ASTFunction* func)
{
    ...
    if (retType->Equals(&VarTypeSimple::IntType)) // Do standard division on integer operands since we return an int.
        return builder.CreateSDiv(d1->CompileRValue(builder, func), d2->CompileRValue(builder, func));
    else if (retType->Equals(&VarTypeSimple::RealType)) // Do division on floating point operands since we return a float.
        return builder.CreateFDiv(d1->CompileRValue(builder, func), d2->CompileRValue(builder, func));
